export const caseStudies = [
    {
      id: 'datamind-ai',
      title: 'DataMind AI',
      subtitle: 'Enterprise Data + AI Platform unifying 7 telecom systems with lakehouse, RAG, and Text-to-SQL',
      ...
      problem: {
        title: 'The Challenge',
        description:
          'A telecom enterprise needed a unified data and AI platform capable of processing millions of daily records across 7 heterogeneous source systems — CRM, Billing, Network, Payments, Recharge, Roaming, and Customer Support — while supporting analytics, ML workflows, and GenAI capabilities without duplicating pipelines.',
        challenges: [
          'Ingesting and harmonizing 7 different source systems with distinct schemas, payload formats, and event cadences — from customer profiles to real-time CDRs and payment transactions',
          'Designing a single lakehouse architecture that serves operational dashboards, ML model training, and GenAI/Text-to-SQL queries without pipeline fragmentation',
          'Implementing full data governance including cataloging, lineage tracking, data quality SLAs, and PII masking across the entire platform',
          'Building a practical Text-to-SQL system with read-only guardrails that lets non-technical users query telecom KPIs in natural language',
          'Supporting RAG-based AI assistants with vector embeddings while keeping every AI-generated answer traceable to source data'
        ]
      },
      solution: {
        title: 'The Solution',
        description:
          'Built an enterprise-grade data + AI platform using a lakehouse medallion architecture (Bronze/Silver/Gold) on Apache Iceberg with MinIO S3 storage, Project Nessie catalog, and Trino federated query engine. The platform integrates 9 Docker Compose profiles covering ingestion, storage, processing, query, orchestration, governance, quality, ML, and AI — all orchestrated via Apache Airflow.',
        architecture: {
          overview: '7 Source Systems → Kafka → NiFi → MinIO Iceberg Bronze → Spark → Silver → Spark → Gold → Trino → Semantic Layer → Power BI / APIs / AI Apps',
          components: [
            {
              name: 'Multi-Source Streaming Ingestion',
              tech: ['Apache Kafka', 'Apache NiFi', 'Confluent Schema Registry'],
              description:
                '9 Kafka topics with 70 partitions ingesting events from 7 simulated enterprise sources. Avro serialization with Schema Registry (BACKWARD compatibility). NiFi micro-batches of 50–128 MB or 5-min windows into MinIO Bronze layer as Parquet.'
            },
            {
              name: 'Iceberg Medallion Lakehouse',
              tech: ['Apache Iceberg', 'Apache Spark', 'MinIO', 'Project Nessie'],
              description:
                'Bronze stores raw Parquet partitioned by year/month/day. Silver cleanses, deduplicates, and enriches with 12 domain-specific tables. Gold delivers 9+ business marts (Customer 360, Revenue, Network Performance, Fraud Monitoring, etc.) with conformed dimensions.'
            },
            {
              name: 'Federated Query Layer',
              tech: ['Trino', 'Nessie', 'Iceberg REST Catalog'],
              description:
                'Trino serves as the central SQL engine over Iceberg Gold tables, resolving metadata via Project Nessie and MinIO. Powers Power BI dashboards, the Semantic Layer, and the AI Text-to-SQL interface.'
            },
            {
              name: 'Semantic & AI Layer',
              tech: ['YAML Semantic Layer', 'Qdrant', 'Ollama/OpenAI', 'LangChain'],
              description:
                'YAML-defined metrics, dimensions, and entities abstract Trino complexity. RAG pipeline: Gold data → vector embeddings → Qdrant vector DB → LLM (Ollama local or OpenAI cloud). Text-to-SQL with read-only validation guardrails operates only on curated Gold datasets.'
            },
            {
              name: 'Orchestration, Governance & Quality',
              tech: ['Apache Airflow', 'OpenMetadata', 'Great Expectations', 'MLflow'],
              description:
                'Airflow orchestrates all Spark ETL, data quality, and metadata sync DAGs. OpenMetadata catalogs assets and lineage. Great Expectations enforces quality gates. MLflow tracks ML experiments across the platform.'
            }
          ]
        },
        keyFeatures: [
          '7-source unified ingestion with Kafka topic-per-entity strategy and Schema Registry at ~8,000 events/sec peak',
          'Full Bronze/Silver/Gold medallion on Iceberg with Nessie Git-like versioning for schema evolution',
          'Trino federated query engine serving Power BI, Semantic Layer, and AI assistant from a single Gold layer',
          'Text-to-SQL with SELECT-only guardrails, Semantic Layer context, and SQL validation before execution',
          'RAG vector pipeline with 5 retrieval modes (Customer, Operations, Executive, Definition, Multi-hop) and evidence-backed citations',
          '9-profile Docker Compose stack for local reproduction of the entire enterprise platform'
        ]
      },
      results: {
        title: 'The Results',
        description:
          'The DataMind AI platform demonstrates how a modern telecom lakehouse can unify analytics, ML, and GenAI workloads on a single architectural foundation — eliminating pipeline duplication while maintaining governance, quality, and auditability.',
        metrics: [
          {
            metric: '7',
            label: 'Source Systems',
            description: 'CRM, Billing, Network, Payments, Recharge, Roaming, Support unified in one platform.'
          },
          {
            metric: '2–5M',
            label: 'Daily Events',
            description: 'Millions of records processed daily across 70 Kafka topic partitions.'
          },
          {
            metric: '9',
            label: 'Gold Marts',
            description: 'Business-ready marts: Customer 360, Revenue, Network, Fraud, Roaming, Support, and more.'
          },
          {
            metric: '3-in-1',
            label: 'Platform Convergence',
            description: 'Analytics + ML + GenAI served from a single lakehouse architecture.'
          }
        ],
        businessImpact: [
          'Replaces fragmented analytics, ML, and AI pipelines with a single governed lakehouse — reducing duplication and maintenance overhead',
          'Enables non-technical stakeholders to query telecom KPIs in natural language via Text-to-SQL with safety guardrails',
          'Accelerates ML adoption with integrated MLflow tracking, feature store patterns, and curated Gold datasets',
          'Improves data trust through OpenMetadata lineage, Great Expectations quality gates, and full audit trails',
          'Provides a reproducible local development environment via 9-profile Docker Compose, lowering onboarding friction for new team members'
        ]
      },
      technicalDetails: {
        technologies: [
          'Apache Kafka', 'Apache NiFi', 'Apache Spark', 'Apache Iceberg', 'MinIO', 'Project Nessie',
          'Trino', 'Apache Airflow', 'OpenMetadata', 'Great Expectations', 'MLflow', 'Qdrant', 'Ollama',
          'OpenAI', 'Python', 'Docker', 'Power BI', 'LangChain'
        ],
        codeSnippets: [
          {
            title: 'Kafka Producer for Telecom CDR Events (Source Simulator)',
            language: 'python',
            code: `from confluent_kafka import Producer, SerializingProducer
from confluent_kafka.schema_registry import SchemaRegistryClient
from confluent_kafka.schema_registry.avro import AvroSerializer

class CDRProducer:
    def __init__(self, topic, schema_str):
    ...
    def flush(self):
        self.producer.flush()`
          ...
          {
            title: 'Spark Bronze to Silver Transformation (Validation & Enrichment)',
            language: 'python',
            code: `from pyspark.sql import SparkSession
from pyspark.sql.functions import col, when, md5, current_timestamp

spark = SparkSession.builder \\
    .config("spark.sql.catalog.my_catalog", "org.apache.iceberg.spark.SparkCatalog") \\
    .config("spark.sql.catalog.my_catalog.type", "hadoop") \\
    .config("spark.sql.catalog.my_catalog.warehouse", "s3a://datamind/iceberg") \\
    .getOrCreate()

bronze_df = spark.read.format("iceberg") \\
    .load("my_catalog.bronze.call_events")

silver_df = bronze_df \\
    .withColumn("call_sk", md5(col("call_id"))) \\
    .withColumn("is_valid", col("duration_seconds").isNotNull() & (col("duration_seconds") > 0)) \\
    .withColumn("event_date", col("timestamp").cast("date")) \\
    .withColumn("processed_at", current_timestamp())

silver_df.write.format("iceberg") \\
    .mode("append") \\
    .save("my_catalog.silver.billing_calls")`
          },
          {
            title: 'Trino SQL — Gold Customer 360 Query via Semantic Layer',
            language: 'sql',
            code: `-- Gold Customer 360 mart queried through Trino
-- Accessible via Power BI, Semantic Layer, or Text-to-SQL
...
LEFT JOIN gold.daily_revenue r ON c.customer_id = r.customer_id
LEFT JOIN gold.customer_usage_daily u ON c.customer_id = u.customer_id
LEFT JOIN gold.payment_analytics p ON c.customer_id = p.customer_id
LEFT JOIN gold.support_analytics s ON c.customer_id = s.customer_id
...
LIMIT 100;`
        ...
        diagrams: [
          {
            title: 'DataMind AI — Enterprise Architecture',
            description: 'Full architecture diagram showing 7 source systems → Kafka → NiFi → Iceberg Medallion → Trino → Semantic Layer → Power BI / AI Apps, with governance, ML, and GenAI paths.',
            imageUrl: '/project_image/DataMind-AI.png'
          },
          {
            title: 'Gold Layer Data Model — Telecom KPIs',
            description: 'Enterprise data model showing Gold layer dimensions and facts for Customer 360, Revenue, Network Performance, Fraud, Roaming, and Support analytics.',
            imageUrl: 'https://raw.githubusercontent.com/MAHMOUDMAMDOH8/DataMind-AI/main/arch/arch.png'
          }
        ]
      },
      github: 'https://github.com/MAHMOUDMAMDOH8/DataMind-AI',
      demo: '',
      lessons: [
        'A single lakehouse architecture can serve analytics, ML, and GenAI workloads without pipeline fragmentation when designed around a clear medallion pattern',
        'Project Nessie with Iceberg provides Git-like versioning that is invaluable for schema evolution in telecom-scale data platforms',
        'Text-to-SQL becomes practical when constrained to a curated Semantic Layer with read-only guardrails — it is not a replacement for governed analytics but a powerful accessibility layer',
        'RAG quality depends far more on the structure of the underlying data model than on LLM choice — clean Gold marts produce reliable AI answers',
        'Docker Compose with 9 profiles strikes a good balance between local reproducibility and production realism for enterprise data platforms'
      ]
    },
    {
      id: 'telecom-data-analytics-hub',
      title: 'Telecom Data Analytics Hub',
      subtitle: 'Kafka â†’ NiFi â†’ PostgreSQL â†’ dbt powering telco-ready facts and dimensions',
  ...
  on from_phone_number = from_user.phone_number`
    ...
    },
    {
      ...
      solution: {
        ...
        architecture: {
          overview: 'Kafka â†’ NiFi â†’ MinIO â†’ Spark-on-Iceberg medallion â†’ ClickHouse galaxy schema for BI',
          components: [
            {
